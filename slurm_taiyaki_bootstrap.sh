...
if [ ! $# == 4 ]; then
  ...
  echo "Usage: $0 [Read directory] [target dir e.g. /tmp/] [pretrained model] [FASTA file]"
  exit
fi
...
prepare_mapped_reads.py --overwrite --alphabet ACGT --mod Y A 6mA $reads $out/modbase.tsv $reads.hdf5 $pretrained_model $fasta_file
