...
if [ ! $# == 1 ]; then
  ...
  echo "Usage: $0 [target dir e.g. /tmp/] "
  exit
fi
...
OMP_NUM_THREADS=40
train_flipflop.py --overwrite --device cpu --mod_factor 0.01 --outdir $out/training /biosw/taiyaki/5.0.1/models/mGru_cat_mod_flipflop.py $out/modbase_new.hdf5
