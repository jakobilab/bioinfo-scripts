...
if [ ! $# == 2 ]; then
  ...
  echo "Usage: $0 [target dir e.g. /tmp/] [CUDA device]"
  exit
fi
...
#train_flipflop.py --overwrite --device $cuda_dev --min_sub_batch_size 48 --mod_factor 0.01 --outdir $out/training /biosw/taiyaki/5.0.1/models/mGru_cat_mod_flipflop.py $out/modbase.hdf5
...
#train_flipflop.py --device $cuda_dev --mod_factor 0.1 --outdir $out/training2 $out/training/model_final.checkpoint modbase.hdf5
#train_flipflop.py --overwrite --device $cuda_dev --min_sub_batch_size 48 --mod_factor 0.1 --outdir $out/training2 $out/training/model_final.checkpoint $out/modbase_new.hdf5


train_flipflop.py --overwrite --device $cuda_dev --chunk_len_min 2000 --chunk_len_max 4000 --size 256 --stride 10 --winlen 31 --min_sub_batch_size 48 --mod_factor 0.1 --outdir $out/training2 $out/training/model_final.checkpoint $out/modbase_new.hdf5
